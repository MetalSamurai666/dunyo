        ...
        <div class="economy__row">
          <div class="economy__small">
            ...
            <div class="economy__more" v-if="false">
              <NuxtLink :to="localePath(`/${cards?.category?.slug}`)">{{
                $t("more_news")
              }}</NuxtLink>
            </div>
          </div>
          ...
          <div class="economy__big">
            <div
              class="economy__poster poster"
              :style="`background-image: url(${encodeURI(
                mainStore.url + '/' + rand?.news?.img
              )})`"
            >
              <div class="economy__title">{{ rand?.category?.title }}</div>
              <div class="poster__text">
                <div class="poster__date">
                  {{ rand?.news?.date.slice(0, 10) }}
                </div>
                <NuxtLink
                  :to="
                    localePath(`/${rand?.category?.slug}/${rand?.news?.slug}`)
                  "
                  class="poster__title"
                  >{{ rand?.news?.title }}</NuxtLink
                >
              </div>
            </div>
          </div>
